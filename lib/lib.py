    ...
    try:
        ...
        for col in df.columns:
            ...
            if row.empty:
                ...
                if new_closest_match[0][1] < closest_match[0][1]:
        ...
        if(closest_match[0][1] <= 3):
            ...
            input = input(f'{name} was not found. Consider {closest_match[0][0]}?(Y/n) ')
            if not input.capitalize().startswith('Y'):
                return getEntityInfo(
                    input(f'Please enter proper entity name for {name}: ')
                )
            else:
                return getEntityInfo(closest_match[0][0])
...
def correctExcel(excel_file_path, output_file_path):
    ...
    for index, row in df.iterrows():
        df.at[index, 0] = getEntityInfo(str(row.iloc[0]))['Data Entity'].astype(str).iloc[0]
...
def correctFolder(datapath, output_datapath, scope_path='res/scope.xlsx'):
    def rename_file(old_filename, new_filename, relative_path):
        old_path = os.path.join(f'{datapath}/{relative_path}', old_filename + '.csv')
        old_path_renamed = os.path.join(f'{datapath}/{relative_path}', new_filename + '.csv')
        
        new_path = os.path.join(f'{output_datapath}/{relative_path}', new_filename + '.csv')
        ...
        if old_filename.lower() != new_filename.lower() and os.path.exists(old_path_renamed):
            ...
            input = input(f'{new_filename} already exists. Rename?(Y/n) ')
            if not input.capitalize().startswith('Y'):
                shutil.move(old_path, os.path.join(duplicated_files, old_filename + '.csv'))
                return
            else:
                shutil.move(old_path_renamed, os.path.join(duplicated_files, new_filename + '.csv'))
        ...
        if scope_df is not None:
            ...
            if(closest_in_scope[0][1] <= 5):
                ...
                if not input.capitalize().startswith('Y'):
                    ...
                    correct_name = closest_in_scope[0][0]
        rename_file(file_name, f'{correct_name}{extension}', relative_path)
