...
def validateStringFields(df, result_df, string_columns_with_size, truncate=True):
    ...
    for string_column_name, size in string_columns_with_size:
        ...
        if len(unique_values_og_df) > len(unique_values_truncated_df):
            print(f"Data loss detected after truncating {string_column_name} to {size} characters.")
            response = input("Do you want to replace the lost values with a standard number sequence? (Y/n): ")
            if not response.upper().startswith('N'):
                ...
                result_df[string_column_name] = [f'{prefix}{i + 1}' for i in range(len(result_df))]
            else:
