...
def validateStringFields(df, result_df, string_columns_with_size, truncate=True):
    ...
    for string_column_name, size in string_columns_with_size:
        ...
        if duplicates.shape[0] > 0 and duplicates.shape[0] != duplicates_og.shape[0]:
            ...
            response = input("Do you want to replace the lost values with a standard number sequence? (Yes/No/Manual/Custom/Ignore): ")            
            if response.upper().startswith('Y'):
                prefix = input("Please enter the constant prefix in the autogenerated sequence: ")
                if f"Old_{string_column_name}" in result_df:
                    result_df[f"Old_{string_column_name}"] = result_df[f"Old_{string_column_name}"].replace('', np.nan)
                    result_df[f"Old_{string_column_name}"].fillna(result_df[string_column_name], inplace=True)
                else:
                    result_df[f"Old_{string_column_name}"] = result_df[string_column_name]
                
                code_mapping = {}
                code_counter = 1
                new_codes = []
                for value in result_df[string_column_name]:
                    if value not in code_mapping:
                        code_mapping[value] = f'{prefix}{code_counter}'
                        code_counter += 1
                    new_codes.append(code_mapping[value])
                result_df[string_column_name] = new_codes
            elif response.upper().startswith('M'):
                if f"Old_{string_column_name}" in result_df:
                    result_df[f"Old_{string_column_name}"] = result_df[f"Old_{string_column_name}"].replace('', np.nan)  # Treat empty strings as NaN
                    result_df[f"Old_{string_column_name}"].fillna(result_df[string_column_name], inplace=True)
                else:
                    result_df[f"Old_{string_column_name}"] = result_df[string_column_name]
                ...
                for _, row in duplicates.iterrows():
                    lost_unique_values.add(row[string_column_name])

                short_names = {}
                for lost_value in lost_unique_values:
                    if lost_value not in short_names:
                        short_name = input(f"Enter a short id for '{lost_value}': ")
                        while short_name.lstrip()[:size].rstrip() in list(truncated_df[string_column_name]):
                            short_name = input(f"'{short_name}' was already taken, please provide another id for '{lost_value}': ")
                        short_names[lost_value] = short_name
                result_df[string_column_name] = result_df[string_column_name].replace(short_names)
            ...
            elif response.upper().startswith('C'):
                if f"Old_{string_column_name}" in result_df:
                    result_df[f"Old_{string_column_name}"] = result_df[f"Old_{string_column_name}"].replace('', np.nan)
                    result_df[f"Old_{string_column_name}"].fillna(result_df[string_column_name], inplace=True)
                else:
                ...
                # Iterate over the lost unique values and apply the truncation strategy
                lost_unique_values = set(duplicates[string_column_name].tolist())
                lost_unique_values = sorted(list(lost_unique_values), key=len, reverse=False)  # Sort by length (longest first)
                new_names = {}
                lowers = []
                for value in lost_unique_values:
                    if len(value) < size: 
                        lowers.append(value.strip())
                        continue
                    truncated_value = value[:size].strip()  # Initial truncation

                    # If the truncated value is already taken, apply the new truncation strategy
                    if truncated_value.lower() in lowers or truncated_value.lower() in truncated_df[string_column_name].str[:size].str.lower().values:
                        for i in range(1, len(value)):
                            truncated_value = value[:i] + value[-(size-i):]
                            if truncated_value.lower() not in lowers and truncated_value.lower() not in truncated_df[string_column_name].str[:size].str.lower().values:
                                break

                    new_names[value] = truncated_value
                    lowers.append(truncated_value.lower())

                result_df[string_column_name] = result_df[string_column_name].replace(new_names)
            else:
                pass
